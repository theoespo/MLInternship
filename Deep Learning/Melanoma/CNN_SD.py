from tensorflow.keras import Model, layers, Input, metrics, backend
...
if data_type == 2:
    ...
    for m in range(len(X_train)):
        ...
        model = Model(inputs=inputs, outputs=outputs)
        model.compile(optimizer="rmsprop", loss="categorical_crossentropy", metrics=["accuracy", metrics.AUC()])
        history = model.fit(X_train[m], y_train[m], epochs=100, batch_size=20, verbose=0, validation_data=(X_test[m], y_test[m]))
        b = time.process_time()
